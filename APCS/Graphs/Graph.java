...
public class Graph<E>{
	...
	public ArrayList<Node<E>> djikstra(Node<E> start, Node<E> end){
		class DjikstraHeuristic implements Heuristic<E>{
			public double calculate(Node<E> n){
				return 0;
			}	
		}
		return AStar(start, end, new DjikstraHeuristic());
	}
	public ArrayList<Node<E>> AStar(Node<E> start, Node<E> end, Heuristic<E> h){
		...
		class SortByWeight implements Comparator<Node<E>>{ 
			public int compare(Node<E> a, Node<E> b){ 
				return dist.get(a) + (int) h.calculate(a) - dist.get(b) - (int) h.calculate(b); 
			} 
		} 
		...
		while(!found){
			...
			for(Node<E> cNode : n.connections().keySet()){
				if(unvisited.contains(cNode)){
					int d = n.distanceTo(cNode) + dist.get(n) + (int) h.calculate(cNode);
					if(d < dist.get(cNode)){
						dist.put(cNode, d);
						last.put(cNode, n);
						pQueue.remove(cNode);
