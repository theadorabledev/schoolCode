...
public class FibonacciHeap<E>{
	...
	private Tree<E> find(E val){
		Tree<E> node = head.getNext();
		while(node != head){
			Tree<E> t = find(node, val);
			node = node.getNext();
		}
				
	}
	private Tree<E> find(Tree<E> parent, E val){
		if(parent.root().equals(val)) return parent;
		for 
